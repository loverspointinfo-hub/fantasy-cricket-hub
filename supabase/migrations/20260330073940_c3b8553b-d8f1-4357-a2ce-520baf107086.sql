...
BEGIN
  SELECT entry_fee, current_entries, max_entries, name, type, match_id, prize_pool, is_guaranteed, prize_breakdown, created_by, COALESCE(max_teams_per_user, 1)
  INTO v_entry_fee, v_current_entries, v_max_entries, v_contest_name, v_contest_type, v_match_id, v_prize_pool, v_is_guaranteed, v_prize_breakdown, v_created_by, v_max_teams_per_user
  ...
  -- Check max teams per user limit
  SELECT COUNT(*) INTO v_user_team_count
  FROM public.contest_entries
  WHERE contest_id = p_contest_id AND user_id = p_user_id;

  IF v_user_team_count >= v_max_teams_per_user THEN
    RAISE EXCEPTION 'Maximum % team(s) allowed per user in this contest', v_max_teams_per_user;
  END IF;
  ...
  IF v_new_entries >= v_max_entries THEN
    ...
    INSERT INTO public.contests (match_id, name, type, entry_fee, prize_pool, max_entries, is_guaranteed, prize_breakdown, created_by, status, current_entries, max_teams_per_user)
    VALUES (v_match_id, v_base_name || ' #' || (v_clone_count + 1), v_contest_type, v_entry_fee, v_prize_pool, v_max_entries, v_is_guaranteed, v_prize_breakdown, v_created_by, 'open', 0, v_max_teams_per_user);
  END IF;
END;
$function$
